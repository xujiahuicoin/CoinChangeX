...
open class CHDepthChartView: UIView {
    ...
    @IBInspectable open var xjh_LineColor: UIColor = UIColor(white: 0.2, alpha: 1) //线条颜色
    ...
    /// 买单在右边
    open var bixjh_artOnDirection: CHKDepthChartOnDirection = .right
    ...
    open var style: CHKLineChartStyle! {           //显示样式
        didSet {
            ...
            self.xjh_LineColor = self.style.xjh_LineColor
            ...
            self.bixjh_artOnDirection = self.style.bixjh_artOnDirection
        }
    ...
    fileprivate func computeDepthValue(for items: [CHKDepthChartItem], type: CHKDepthChartItemType) {
        ...
        if self.style.bixjh_artOnDirection == .left{
            if type == .bid {
                //买单深度是由价格大到小地累计
                start = items.count - 1
                end = 0
    ...
    func setSelectedIndexByPoint(_ point: CGPoint) {
        ...
        // 判断买单还是卖单
        if self.bixjh_artOnDirection == .left{
...
extension CHDepthChartView {
    ...
    fileprivate func drawChartFrame() {
        ...
        borderLayer.fillColor = self.xjh_LineColor.cgColor // 闭环填充的颜色
    ...
    func drawChart() -> [(CGRect, String)]{
        ...
        if self.bixjh_artOnDirection == .right{
            ...
            if let bixjh_artLayer = self.drawDepthChart(items: self.bidItems, startIndex: startIndex, strokeColor: self.bidColor.stroke, fillColor: self.bidColor.fill, lineWidth: self.bidColor.lineWidth) {
                self.drawLayer.addSublayer(bixjh_artLayer)
        ...
        //        //绘制买方深度图层
        //        if let bixjh_artLayer = self.drawDepthChart(items: self.bidItems, startIndex: 0, strokeColor: self.bidColor.stroke, fillColor: self.bidColor.fill, lineWidth: self.bidColor.lineWidth) {
        //            self.drawLayer.addSublayer(bixjh_artLayer)
        //            startIndex = self.bidItems.count
        //        }
        //
